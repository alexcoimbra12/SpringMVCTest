      	...
      	<c:otherwise>
      		<form method="POST">
		      	...
		      	<c:forEach items="${usuarioList}" var="usuario">
					...
					<td>${usuario.user}</td>
					<td><button type="submit" value="${usuario.id}" id="deletar" name="deletar" formaction="${deletarUsuario}" onclick="if (!confirm('Deseja realmente excluir ${usuario.nome}?')) return false">Deletar</button></td><tr>
		        </c:forEach>
      			</table>
      		</form>	
      	</c:otherwise>
